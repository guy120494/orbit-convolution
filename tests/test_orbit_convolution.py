...
from layers.OrbitConvolution import OrbitSumConvolution


class OrbitConvolutionTest(tf.test.TestCase):

    def test_invariance_in_x_axis(self):
        layer = OrbitSumConvolution(kernel_size=3, num_filters=1)
    ...
    def test_invariance_in_y_axis(self):
        layer = OrbitSumConvolution(kernel_size=3, num_filters=1, axis=1)
        x_train, _, _, _ = get_mnist_data()
        sample = x_train[0]
        sample = np.reshape(sample, [1, *sample.shape])
        sample = tf.convert_to_tensor(sample)
