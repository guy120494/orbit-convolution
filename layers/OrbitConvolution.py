...
class OrbitLogSumExpConvolution(Layer):
    def __init__(self, kernel_size: int, num_filters: int, axis: int = 0, **kwargs):
        self.axis = axis
        self.kernel_size = kernel_size
        self.num_filters = num_filters
        self.kernel = None
        super(OrbitLogSumExpConvolution, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        super(OrbitLogSumExpConvolution, self).build(input_shape)

    def call(self, x, **kwargs):
        x = tf.math.reduce_logsumexp(x, axis=self.axis + 1)
